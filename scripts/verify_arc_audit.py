...
def main():
    ...
    print("\n--- ARC1 ---")
    if not check_file_exists("backend/main.py", "FastAPI Entry Point"):
        failed += 1
    if not check_content("backend/main.py", "other_asgi_app", "Socket.IO ASGI Wrapper"):
        failed += 1
    if not check_content("docs/architecture.md", "Backend API Architecture", "Architecture Docs"):
        failed += 1
    ...
    # ARC4: Best Practices
    print("\n--- ARC4 ---")
    if not check_content("backend/api/models/health.py", "BaseModel", "Pydantic Models"):
        failed += 1
