...
## What Works
✅ History data loads successfully (96 slots)
✅ Race condition fixed - critical path fetching works
✅ `actual_soc` values are present in the data
✅ 188 total slots displayed (96 history + 92 tomorrow)

## What's Broken
❌ Planned action fields missing from history slots
❌ ChartCard shows only observations, not the planned schedule overlay
...
**Analysis:**
- ✅ Has observations: `actual_charge_kw`, `actual_soc`, `actual_export_kwh`
- ❌ Missing planned actions: `charge_kw`, `battery_discharge_kw`, `target_soc`
- ❌ Missing UI fields: `reason`, `priority`, `is_historical`
...
- Data includes `actual_soc` ✅
- Data missing planned overlay fields ❌
...
1. Load observations from DB (current behavior ✅)
2. Load today's planned schedule from `schedule.json` (missing ❌)
...
## Success Criteria
After fix:
- ✅ curl shows `target_soc`, `charge_kw`, `reason` in history slots
- ✅ ChartCard displays green/red action bars
- ✅ Target SoC line overlay visible
- ✅ Reason labels appear on hover
- ✅ Water heating indicators show when applicable

## Previous Context
**Version:** v2.4.10-beta
**Fixed:** Race condition where `historySlots` loaded after `slotsOverride` computation
**Current issue:** History loads early (✅) but contains incomplete data (❌)
